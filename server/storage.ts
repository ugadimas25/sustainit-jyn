...
export class DatabaseStorage implements IStorage {
  ...
  async calculatePeatlandIntersection(geometry: any): Promise<{ intersectionArea: number; peatlandData?: any[] }> {
    try {
      // Mock peatland intersection calculation for now
      // In a real implementation, this would use PostGIS to calculate intersection with gambut_idn_new table

      // Simulate some plots having peatland intersection based on coordinates
      if (geometry && geometry.coordinates) {
        ...
        if (coords && coords.length > 0) {
          ...
          // Simulate peatland areas in Indonesia (rough coordinates)
          // Sumatra peatland areas: 100-104°E, 0-6°S
          // Kalimantan peatland areas: 108-118°E, 3°S-3°N
          if ((lng >= 100 && lng <= 104 && lat >= -6 && lat <= 0) ||
              (lng >= 108 && lng <= 118 && lat >= -3 && lat <= 3)) {

            // Simulate intersection area (10-50% of plot area)
            const intersectionPercentage = Math.random() * 0.4 + 0.1; // 10-50%

            // Calculate approximate polygon area in hectares (rough estimation)
            const areaHa = this.calculatePolygonAreaHa(coords);
            const intersectionArea = areaHa * intersectionPercentage;

            console.log(`🌿 Peatland intersection detected: ${intersectionArea.toFixed(4)}ha (${(intersectionPercentage * 100).toFixed(1)}% of plot)`);

            return {
              intersectionArea,
              peatlandData: [{
                nama_khg: 'Kesatuan Hidrologi Gambut Sample',
                kode_khg: 'KHG-001',
                kubah_gbt: Math.random() > 0.5 ? 'Kubah Gambut' : 'Non Kubah Gambut',
                luas_ha: intersectionArea
              }]
            };
          }
        }
      ...
      console.error('Error calculating peatland intersection:', error);
      return { intersectionArea: 0 };
    }
  }

  private calculatePolygonAreaHa(coordinates: number[][]): number {
    ...
    // 1 degree ≈ 111 km at equator, so 1 square degree ≈ 12321 km² = 1232100 ha
    const areaHa = area * 1232100;

    // Cap the result to reasonable values (0.1 to 1000 ha)
    return Math.max(0.1, Math.min(1000, areaHa));
  }
