...
export async function registerRoutes(app: Express): Promise<Server> {
    ...
    try {
      ...
      const defaultRiskItems = [
        // Spatial Risk Analysis items from Excel
        {
          ...
          riskLevel: "low",
          parameter: "Source TBS From Plantations Developed Before December 2020",
          riskValue: 3,
          weight: "45.00",
          ...
          mitigationDescription: "Periodic monitoring of TBS sources",
        ...
        },
        {
          ...
          itemName: "Land Legality",
          riskLevel: "low",
          parameter: "Has Permit and Located in APL Area",
          riskValue: 3,
          weight: "35.00",
          ...
          mitigationRequired: false,
          mitigationDescription: "Periodic Monitoring of TBS Sources",
          dataSources: ["WDPA Map", "Forest Area Map Indonesia"],
        ...
        },
        {
          ...
          itemName: "Peatland Area",
          riskLevel: "medium",
          parameter: "TBS Source Plots overlap with indicative peatland map for protection functions and are undergoing technical guidance",
          riskValue: 2,
          weight: "10.00",
          ...
          mitigationDescription: "Company socialization of policies to suppliers",
          dataSources: ["Peatland Area Map"],
        ...
        },
        {
          ...
          riskLevel: "low",
          parameter: "No Overlap and Has SOP Regarding Handling Stakeholder Complaints",
          riskValue: 3,
          weight: "10.00",
          ...
          finalScore: 0.03,
          mitigationRequired: false,
          mitigationDescription: "Periodic monitoring of social issues for early detection of potential conflicts",
          dataSources: ["Indigenous People Map"],
  ...
  app.get('/api/generate-dummy-dds-pdf', isAuthenticated, async (req, res) => {
    try {
      ...
      doc.setFont('helvetica', 'normal');
      doc.text(dummyReport.companyInternalRef || 'DDS-2024-001', 82, yPos + 8);
      doc.text(dummyReport.activity || 'Import of Palm Oil Products', 82, yPos + 20);
      yPos += 35;
      ...
      doc.setFont('helvetica', 'normal');
      doc.text(dummyReport.operatorLegalName || 'KPN Corporation Berhad', 62, yPos + 8);
      const address = dummyReport.operatorAddress || 'Level 6, Menara KPN, Jalan Sultan Ismail, Kuala Lumpur, Malaysia';
      const addressLines = doc.splitTextToSize(address, 135);
      doc.text(addressLines, 62, yPos + 16);
      doc.text(dummyReport.operatorCountry || 'Malaysia', 62, yPos + 32);
      yPos += 45;
      ...
      doc.setFont('helvetica', 'normal');
      doc.text(dummyReport.productDescription || 'Crude Palm Oil (CPO)', 12, yPos + 20);
      doc.text(dummyReport.netMassKg?.toString() || '2150.000', 72, yPos + 20);
      doc.text(dummyReport.percentageEstimation?.toString() + '%' || '5%', 122, yPos + 20);
      doc.text(dummyReport.supplementaryUnit || 'MT', 152, yPos + 20);

      // Scientific details
      doc.text(`Scientific: ${dummyReport.scientificName || 'Elaeis guineensis'}`, 12, yPos + 28);
      doc.text(`Common: ${dummyReport.commonName || 'Oil Palm'}`, 72, yPos + 28);
      yPos += 45;
      ...
      doc.setFont('helvetica', 'normal');
      doc.text(dummyReport.totalProducers?.toString() || '15', 55, yPos + 8);
      doc.text(dummyReport.totalPlots?.toString() || '45', 40, yPos + 20);
      doc.text(dummyReport.totalProductionArea?.toString() || '1250.50', 140, yPos + 8);
      doc.text(dummyReport.countryOfHarvest || 'Malaysia', 150, yPos + 20);
  ...
  app.post('/api/geojson/upload', isAuthenticated, async (req, res) => {
    try {
      ...
      if (analysisResults.data?.features) {
        ...
        // Clear existing results before processing new upload
        try {
          await storage.clearAnalysisResults();
          console.log("✓ Cleared existing analysis results");
        } catch (error) {
          console.warn("Warning: Could not clear existing results:", error);
          // Continue processing even if clear fails
        }
  ...
  app.delete('/api/analysis-results', isAuthenticated, async (req, res) => {
    try {
      await storage.clearAnalysisResults();
      res.json({ success: true, message: 'Analysis results cleared successfully' });
    } catch (error) {
