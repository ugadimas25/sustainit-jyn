          ...
          <script>
            ...
            const deforestationLayers = {
              gfw: L.tileLayer('https://tiles.globalforestwatch.org/umd_tree_cover_loss/v1.12/dynamic/{z}/{x}/{y}.png?start_year=2021&end_year=2024&tree_cover_density_threshold=30&render_type=true_color', {
                ...
                opacity: 0.8,
                maxZoom: 18,
                className: 'gfw-deforestation-layer'
              }),
              jrc: L.tileLayer.wms('https://ies-ows.jrc.ec.europa.eu/iforce/gfc2020/wms.py', {
                ...
                version: '1.3.0',
                maxZoom: 18,
                className: 'jrc-forest-layer'
              }),
              sbtn: L.tileLayer('https://tiles.globalforestwatch.org/sbtn_natural_lands/latest/dynamic/{z}/{x}/{y}.png?render_type=true_color', {
                attribution: '© SBTN - Science Based Targets Network',
                opacity: 0.8,
                maxZoom: 18,
                className: 'sbtn-natural-layer',
                errorTileUrl: 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNkYPhfDwAChwGA60e6kgAAAABJRU5ErkJggg=='
              })
            };
            ...
            setTimeout(() => {
              ...
              // Test SBTN service with multiple endpoints
              const sbtnTestUrls = [
                'https://tiles.globalforestwatch.org/sbtn_natural_lands/latest/dynamic/6/32/21.png',
                'https://gis-development.koltivaapi.com/data/v1/gee/tiles/sbtn_natural_lands/6/32/21'
              ];

              sbtnTestUrls.forEach((url, index) => {
                fetch(url)
                  .then(response => {
                    if (response.ok) {
                      console.log(`✅ SBTN service ${index + 1} is accessible: ${url}`);
                    } else {
                      console.warn(`⚠️ SBTN service ${index + 1} returned: ${response.status} for ${url}`);
                    }
                  })
                  .catch(error => {
                    console.warn(`❌ SBTN service ${index + 1} error: ${error.message} for ${url}`);
                  });
              });
